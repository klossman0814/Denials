...
function parse837(content) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    switch (segId) {
      ...
      case 'SBR': {
        subscriber = getContext('22');
        subscriber.relationship_code = (elements[2] || '').trim();
        subscriber.claim_filing_type = (elements[9] || '').trim();
        if (currentClaim) {
          currentClaim.subscriber_relationship_code = currentClaim.subscriber_relationship_code || subscriber.relationship_code;
          currentClaim.claim_filing_type = currentClaim.claim_filing_type || subscriber.claim_filing_type;
        }
        break;
      }
        ...
        if (currentClaim) {
          switch (dateQual) {
            ...
            case '096': // Discharge Hour (837I)
              currentClaim.discharge_hour = (elements[3] || '').trim();
              break;
          }
        }
        break;
